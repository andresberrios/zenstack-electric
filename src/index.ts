 ...
 * WHERE clause by walking the schema's expression AST directly.
 *
...
export function compilePolicyWhere(
  ...
  const ctx: CompileContext = { schema, auth: auth ?? {}, params: [] }
...
function compileMember(
  ...
  if (receiver.kind === 'call' && (receiver.function as string) === 'auth') {
    const value = resolveAuthValue(ctx.auth, members)
    return addParam(ctx, value)
  }
...
function resolveAuthValue(auth: object, members: string[]): unknown {
